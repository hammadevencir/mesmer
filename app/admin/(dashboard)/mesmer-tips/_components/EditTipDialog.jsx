...
const EditTipDialog = ({ children, tip, onUpdate }) => {
  const [open, setOpen] = useState(false);
  const [selectedCategory, setSelectedCategory] = useState(tip?.category || "School & Exams");
  ...
  useEffect(() => {
    if (open && tip) {
      setSelectedCategory(tip.category || "School & Exams");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        {/* Footer */}
        <div className="px-4 max-w-[570px] w-full py-5 border-t border-[#F3F4F6] flex gap-3 bg-white shrink-0">
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full">
            <Button
              variant="outline"
              onClick={() => setOpen(false)}
              disabled={saving}
              className="w-full flex-1 h-[54px] rounded-full border-[#8F00FF] text-[#8F00FF] hover:bg-[#F3E8FF] hover:text-[#8F00FF] text-[18px] font-bold"
              style={{
                fontFamily: "'Inter Display', var(--font-inter), sans-serif",
              }}
            >
            ...
            <Button
              ...
              className="w-full flex-1 h-[54px] rounded-full bg-[#8F00FF] hover:bg-[#7a00d9] text-white text-[18px] font-bold whitespace-nowrap"
              style={{
                fontFamily: "'Inter Display', var(--font-inter), sans-serif",
              }}
            >
